                    ...
                    if (!merch) {
                        if (slea.available() > 0) {
                            if (!WorldConstants.JZSD) {
                                ...
                                c.getPlayer().dropMessage(1, "當前伺服器無法使用精靈商人。");
                            }
                        }
                    } else {
                ...
                if (merch) {
                    ...
                    if (pack == null) {
                        ...
                        if (ServerConfig.LOG_MRECHANT) {
                            FileoutputUtil.logToFile("logs/Data/精靈商人領回.txt", FileoutputUtil.NowTime() + "帳號角色名字:" + c.getAccountName() + " " + c.getPlayer().getName() + " 從精靈商人取回楓幣: " + pack.getMesos() + " 和" + pack.getItems().size() + "件物品[" + output + "]\r\n");
                        }
                        c.getPlayer().setConversation(0);
                    } else {
