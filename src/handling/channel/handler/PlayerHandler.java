...
public class PlayerHandler {
    ...
    public static final void UseChair(final int itemId, final MapleClient c, final MapleCharacter chr) {
        ...
        if (toUse == null && itemId >= 3010000 && itemId < 9999999) {
            ...
            World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[封鎖系統] " + c.getPlayer().getName() + " 因為修改封包而被管理員永久停權。"));
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密語]  " + c.getPlayer().getName() + "(" + c.getPlayer().getId() + ") 修改椅子(" + itemId + ")封包，坐上椅子時封鎖。 身上並沒有該物品"));
...
//        }
    ...
    public static final void CancelChair(final short id, final MapleClient c, final MapleCharacter chr) {
        ...
        if (id == -1) { // Cancel Chair
            if (/*chr.getChair() == 3010001*/chr.getChair() / 10000 == 301 && GameConstants.isFishingMap(chr.getMapId())) {
...
//			chr.getMap().broadcastMessage(chr, MaplePacketCreator.showMagnet(mobId, slea.readByte()), chr.getPosition());
    ...
    public static final void closeRangeAttack(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr, final boolean energy) {
        ...
        if (attack.skill != 0) {
            if (attack.skill == 海盜.雙子星攻擊 || attack.skill == 槍手.散射 || attack.skill == 槍手.脫離戰場 || attack.skill == 神槍手.三連發 || attack.skill == 神槍手.寒霜噴射 || attack.skill == 神槍手.火焰噴射 || attack.skill == 神槍手.指定攻擊 || attack.skill == 槍神.瞬迅雷 || attack.skill == 槍神.心靈控制 || attack.skill == 盜賊.雙飛斬 || attack.skill == 刺客.吸血術 || attack.skill == 暗殺者.風魔手裏劍 || attack.skill == 暗殺者.楓幣攻擊 || attack.skill == 夜使者.挑釁 || attack.skill == 暗影神偷.挑釁 || attack.skill == 夜使者.三飛閃 || attack.skill == 暗夜行者1.雙飛斬 || attack.skill == 暗夜行者3.風魔手裏劍 || attack.skill == 暗夜行者3.三飛閃 || attack.skill == 破風使者1.二連箭 || attack.skill == 破風使者3.箭雨 || attack.skill == 破風使者3.四連箭 || attack.skill == 破風使者3.疾風掃射 || attack.skill == 破風使者3.疾風光速神弩 || attack.skill == 破風使者3.暴風神射 || attack.skill == 弓箭手.二連箭 || attack.skill == 弓箭手.斷魂箭 || attack.skill == 獵人.炸彈箭 || attack.skill == 遊俠.四連箭 || attack.skill == 遊俠.烈火箭 || attack.skill == 遊俠.箭雨 || attack.skill == 箭神.龍魂之箭 || attack.skill == 箭神.暴風神射 || attack.skill == 弩弓手.穿透之箭 || attack.skill == 狙擊手.四連箭 || attack.skill == 狙擊手.升龍弩 || attack.skill == 狙擊手.寒冰箭 || attack.skill == 神射手.龍魂之箭 || attack.skill == 神射手.必殺狙擊 || attack.skill == 神射手.光速神弩) {
                c.getSession().close();
                return;
            }
            if (chr.getMapId() == 109010000 || chr.getMapId() == 109030001 || chr.getMapId() == 109060000 || chr.getMapId() == 109040000) {
                chr.dropMessage(5, "該地圖無法使用技能");
                c.sendPacket(MaplePacketCreator.enableActions());
                return;
            }

            if (attack.skill == 1221011 && (chr.getMapId() >= 211060000 && chr.getMapId() <= 211070200)) {
                chr.dropMessage(5, "該地圖無法該使用技能");
                c.sendPacket(MaplePacketCreator.enableActions());
                return;
            }
        ...
        if (!energy) {
            ...
            if (isFinisher(attack.skill)) {
                ...
                maxdamage = 20000000; // FIXME reenable damage calculation for finishers
            }
        }
    ...
    public static final void Heal(final LittleEndianAccessor slea, final MapleCharacter chr) {
        ...
        if (healMP != 0 && chr.canMP()) {
            ...
            if (healMP > check * 5) {
                //  chr.getCheatTracker().registerOffense(CheatingOffense.REGEN_HIGH_MP, String.valueOf(healMP) + " 服務端:" + stats.getHealMP());
...
//            slea.skip(6); //D3 75 00 00 00 00
        ...
        // } catch (Exception ex) {
        //     System.err.println("切換地圖異常" + ex);
        //    FileoutputUtil.outError("logs/切換地圖異常.txt", ex);
        // }
    }
...
//	slea.readShort(); // Original Y pos
    ...
    /*public static final void UpdateFkCharMessages(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
    ...
    public static final void handleLogout(final LittleEndianAccessor slea, MapleClient c) {
        ...
        if (ACname != null) {
            ...
            try (Connection con = DBConPool.getInstance().getDataSource().getConnection(); PreparedStatement ps = con.prepareStatement(sb.toString())) {
