...
public class NPCHandler {
    ...
    public static final void Storage(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (chr.hasBlockedInventory2(true)) { //hack
            c.getPlayer().dropMessage(1, "系統錯誤，請聯繫管理員。");
            c.sendPacket(MaplePacketCreator.enableActions());
            return;
        }
        ...
        switch (mode) {
                ...
                if (c.getPlayer().CanStorage()) {
                    if (item != null) {
                        if (!MapleInventoryManipulator.checkSpace(c, item.getItemId(), item.getQuantity(), item.getOwner())) {
                            ...
                            FileoutputUtil.logToFile("logs/Data/倉庫操作.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 帳號: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了倉庫操作拿出物品:" + item.getItemId() + " 數量：" + item.getQuantity());
                            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密語] " + " 帳號: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了倉庫操作拿出物品:" + item.getItemId() + " 數量：" + item.getQuantity()));
                        }
                    } else {
                        c.sendPacket(MaplePacketCreator.enableActions());
                    ...
                    c.getPlayer().dropMessage(1, "操作過快請稍後在嘗試。");
                    storage.store(item);
                }
                break;
            }
            case 5: { // 倉庫處理
                ...
                if (itemId == 4110010) {
                    c.getPlayer().dropMessage(1, "無法存入該物品。");
                    c.sendPacket(MaplePacketCreator.enableActions());
                    return;
                }
                ...
                if (c.getPlayer().CanStorage()) {
                    ...
                    if (chr.getMeso() < 100 || chr.getMapId() == 910000000 && chr.getMeso() < 500) {
                        ...
                        if (item.getItemId() == itemId && (item.getQuantity() >= quantity || GameConstants.isThrowingStar(itemId) || GameConstants.isBullet(itemId))) {
                            ...
                            //chr.saveToDB(false, false);
                            FileoutputUtil.logToFile("logs/Data/倉庫操作.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 帳號: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了倉庫操作存入物品:" + item.getItemId() + " 數量：" + item.getQuantity());
                            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密語] " + " 帳號: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了倉庫操作存入物品:" + item.getItemId() + " 數量：" + item.getQuantity()));
                        } else {
            ...
            case 7: {
                ...
                if (c.getPlayer().CanStorage()) {
                    if ((meso > 0 && storageMesos >= meso) || (meso < 0 && playerMesos >= -meso)) {
                        ...
                        FileoutputUtil.logToFile("logs/Data/倉庫操作.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 帳號: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了倉庫操作金幣數量:" + meso);
                        World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密語] " + " 帳號: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了倉庫操作金幣數量:" + meso));

                        if (meso >= 50000000 || meso <= -50000000) {
                            FileoutputUtil.logToFile("logs/Data/倉庫大額金幣操作.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 帳號: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了倉庫大額金幣操作數量:" + meso);
                            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密語] " + " 帳號: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用了倉庫大額金幣操作數量:" + meso));
                        }
