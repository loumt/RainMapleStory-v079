...
public class MapleGuildRanking {
    ...
    private void showJzRank(int mapid) {
        ranks3.clear();
        ResultSet rs;
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection(); PreparedStatement ps = con.prepareStatement("SELECT * FROM jzlog WHERE mapid = " + mapid + " ORDER BY `meso` DESC LIMIT 10")) {
            rs = ps.executeQuery();
            while (rs.next()) {
                final JzRankingInfo rank3 = new JzRankingInfo(
