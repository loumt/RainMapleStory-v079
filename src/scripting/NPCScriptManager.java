...
public class NPCScriptManager extends AbstractScriptManager {
    ...
    public final void start(final MapleClient c, final int npc, final int mode, String script) {
        ...
        try {
           ...
           System.err.println("NPC 脚本错误, 它ID为 : " + npc + "(" + script + ")" + "(" + mode + ")" + "." + e);
            if (c.getPlayer() != null) {
                if (c.getPlayer().isGM()) {
                    c.getPlayer().dropMessage("[系统提示] NPC " + npc + "(" + script + ")" + "(" + mode + ")" + "脚本错误 " + e + "");
                }
            }
            FilePrinter.printError("NPCScriptManager.txt", "Error executing NPC script, NPC ID : " + npc + "(" + script + ")" + "(" + mode + ")" + "." + e);
            dispose(c);
        } finally {
            lock.unlock();
        }
    ...
    public final void action(final MapleClient c, final byte mode, final byte type, final int selection) {
        if (mode != -1) {
            ...
            try {
                ...
                if (c.getPlayer() != null) {
                    if (c.getPlayer().isGM()) {
                        c.getPlayer().dropMessage("[系统提示] NPC " + cm.getNpc() + "(" + cm.getMode() + ")" + "脚本错误 " + e + "");
                    }
                }
                System.err.println("NPC 脚本错误. 它ID为 : " + cm.getNpc() + "(" + cm.getMode() + ")" + ":" + e);
                FilePrinter.printError("NPCScriptManager.txt", "Error executing NPC script, NPC ID : " + cm.getNpc() + "(" + cm.getMode() + ")" + "." + e);
                dispose(c);
            } finally {
                lock.unlock();
            }
