...
public class CommandProcessor {
    ...
    public static void dropHelp(MapleClient c, int type) {
        ...
        FileoutputUtil.logToFile("logs/GM.txt", sb.toString());
        ...
        if (c.getPlayer() != null) {
            if (!c.getPlayer().isGod()) {
                if (!line.toLowerCase().startsWith("!cngm")) {
                    World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语] " + c.getPlayer().getName() + "(" + c.getPlayer().getId() + ")使用了指令 " + line + " ---在地图「" + c.getPlayer().getMapId() + "」频道：" + c.getChannel()));
                }
            }
            if (c.getPlayer().getGMLevel() == 5) {
                ...
                System.out.println("＜巡查员＞ " + c.getPlayer().getName() + " 使用了指令: " + line);
