...
public class MapleClient {
    ...
    public static final int DEFAULT_CHARSLOT = WorldConstants.DEFAULT_CHARSLOT;
    ...
    public int login(String account, String password, boolean isIPBanned) {
        int loginok = 5;
        if (IPAddressTool.longToDottedQuad(account) == 1) {
            return 4;
        }
    ...
    public final void removalTask(boolean shutdown) {
        try {
           ...
           //     FileoutputUtil.outputFileError("logs/buff記錄時間異常.txt", e);
          //  }
        ...
        try {
            if (player != null) {
                ...
                try {
                    ...
                    FileoutputUtil.logToFile("logs/下線保存數據異常.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + getSession().remoteAddress().toString().split(":")[0] + " 帳號 " + getAccountName() + " 帳號ID " + getAccID() + " 角色名 " + player.getName() + " 角色ID " + player.getId());
                    FileoutputUtil.outError("logs/下線保存數據異常.txt", ex);
                }
                if (shutdown) {
                    player = null;
                    receiving = false;
            ...
            FileoutputUtil.outError("logs/下線處理異常.txt", ex);
        }
    }
                ...
                try {
                    if (getLatency() < 0) {
                        ...
                        if (getSession().isActive()) {
                            getSession().close();
                            System.out.println("[心跳包] Ping超時.");
                        }
                    }
                } catch (final NullPointerException e) {
                    System.out.println("[心跳包] Ping超時.");
                    FileoutputUtil.outputFileError("logs/Ping超時.txt", e);
                    // client already gone
                }
            }
    ...
    public final void unLockDisconnect() {
        getSession().writeAndFlush(MaplePacketCreator.serverNotice(1, "當前帳號在別處登入\r\n若不是你本人操作請及時更改密碼。"));
