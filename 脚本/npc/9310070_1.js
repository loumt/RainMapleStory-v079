...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1) status++;
        if (status == 0) {
            cm.sendSimple("\r\n#L0##r<高级强化+3> 30张#v4000313##b增加装备砸卷次数#l");
        } else if (status == 1) {
            if (selection == 0) { 
                fstype = 1;
                cm.sendNext("你目前选择的是30张#v4000313##b增加装备砸卷次数#k\r\n\r\n#r注意：50%机率成功");
            } else if (selection == 1) {
                fstype = 2;
            ...
            if (fstype == 13) {
               if (cm.haveItem(4000313, 30) == false) {
                    cm.sendOk("对不起,你没有足够的#v4000313#升级");
                    cm.dispose();
                    return;
                }
				...
				cm.gainItem(4000313, -30);
                    ...
                    cm.worldMessage("[装备提升]：恭喜[" + cm.getChar().getName() + "]在装备管理处,使用黄金枫叶为武器提升了3次砸卷次数");
                    cm.dispose();
                } else {
                    cm.gainItem(4000313, -10);
                    cm.sendOk("真遗憾，升级失败，扣除手续费10张黄金枫叶");
                }
                cm.dispose();
                return;
            }
