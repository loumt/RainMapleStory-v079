...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			cm.sendYesNo("我是#r技能全满#k商人，您确定需要享受这项服务?");
		} else if (status == 1) {
                if (cm.getMeso() >=0){
			...
			cm.teachSkill(4221008,5,5); //End of max-level "1" skills
			...
			cm.teachSkill(5121008,5,5); //Pirate's Rage - Allows you to break out of an abnormal state. As its level increases, the number of abnormal states that can be broken out increases. \n#cWaiting time : 10 min.#
			...
			cm.teachSkill(5221010,5,5); //Speed Infusion - Uses HP and MP to temporarily increase the attacking speed of a weapon. This can be combined with other boosters, and everyone in the party will be positively affected by this.\nRequired Skill : #cLevel 20 of Knuckler Booster#
